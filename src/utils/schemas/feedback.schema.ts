...
export const feedbackSchema = z.object({
	firstname: z
		...
		.max(FIRSTNAME_MAX_LENGTH, `Максимальная длина имени - ${FIRSTNAME_MAX_LENGTH}`),
	lastname: z
		.string()
		.regex(/^[А-Яа-я][а-я]*$/, 'Введите настоящую фамилию')
		.min(2, 'Минимальная длина - 2')
		.max(LASTNAME_MAX_LENGTH, `Максимальная длина фамилии - ${LASTNAME_MAX_LENGTH}`),
	email: z
		.string()
	...
	message: z
		.string()
		.max(FEEDBACK_MAX_LENGTH, `Максимальная длина сообщения - ${FEEDBACK_MAX_LENGTH}`)
})
